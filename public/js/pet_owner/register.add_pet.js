...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addPetButton && petCardsContainer) {
        addPetButton.addEventListener('click', function() {
            ...
            // ★ Pet 1 の削除ボタンを表示する
            const firstPetCard = document.getElementById('petCard1');
            const firstDeleteButton = firstPetCard.querySelector('.delete-pet-button');
            if (firstDeleteButton) {
    ...
    // ページロード時に既存のPet 1の削除ボタンにはイベントリスナーを設定しておく
    attachDeleteListeners();
}); // DOMContentLoadedの閉じ